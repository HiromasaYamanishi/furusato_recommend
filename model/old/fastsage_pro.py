...
from torch.multiprocessing import JoinableQueue, Lock, Process
...
class FastSAGEPro(torch.nn.Module):
    ...
    def stageOne(self, user: torch.Tensor, pos: torch.Tensor, neg: torch.Tensor):
            ...
            producers.append(
                Process(
                    ...
                    args=(
                        queue,
                        lock,
                        user[process_unit * i : process_unit * (i + 1)],
                        pos[process_unit * i : process_unit * (i + 1)],
                        neg[process_unit * i : process_unit * (i + 1)],
                    ),
                )
            )
        ...
        for j in range(self.consumer_process_num):
            consumers.append(
                Process(target=self.consumer, args=(queue, lock), daemon=True)
            )
