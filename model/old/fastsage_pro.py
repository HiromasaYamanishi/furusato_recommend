...
import time
from torch.multiprocessing import Process,Lock,JoinableQueue
...
class FastSAGEPro(torch.nn.Module):
    ...
    def get_layer_indice_offset(self, neighbors, offsets):
        ...
        for layer in range(self.num_layers):
            target_tmp = torch.arange(begin_node_offset, begin_node_offset+batch_size*(neighbor_num**layer))
    ...
    def forward(self, neighbors, offsets, mode):
        ...
        for i in range(self.num_layers):
            ...
            aggregated = scatter(source_x, target_layer.to(self.device), out=out.to(self.device), dim=0)
            x = self.w_linears[i](torch.cat([x[:layer_offsets[i+1]], aggregated], dim=1))
    ...
    def producer(self, queue, lock, user, pos, neg):
        ...
        #with lock:
        queue.put((user_neighbors, user_offsets, pos_neighbors, pos_offsets, neg_neighbors, neg_offsets))
        #print('put', user_neighbors)
        queue.join()
    ...
    def consumer(self, queue, lock):
        ...
        while True:
            #try:
                #with lock:
            user_neighbors, user_offsets, pos_neighbors, pos_offsets, neg_neighbors, neg_offsets = queue.get()
            #print(user_neighbors, user_offsets, pos_neighbors, pos_offsets, neg_neighbors, neg_offsets)
    ...
    def stageOne(self, user:torch.Tensor, pos: torch.Tensor, neg: torch.Tensor):
        ...
        producers, consumers = [], []
        process_unit = len(user)//self.producer_process_num
        for i in range(self.producer_process_num):
            producers.append(Process(target=self.producer, args=(queue, lock, user[process_unit*i:process_unit*(i+1)], pos[process_unit*i:process_unit*(i+1)], neg[process_unit*i:process_unit*(i+1)])))
            
        for j in range(self.consumer_process_num):
            consumers.append(Process(target=self.consumer, args=(queue, lock), daemon=True))
