...
from neighbor_sampling import UniformNeighbors
import time
...
class FastSAGEPar(torch.nn.Module):
    def __init__(self, config, dataset):
        ...
        self.process_num = 16
        self.neighbor_sampler = UniformNeighbors(dataset, self.num_layers, self.num_neighbors)
    ...
    def get_initial_hiddens(self, neighbors, mode):
        ...
        for i in range(len(neighbors)):
            if offset%2:
                initial_hiddens.append(item_initial_embedding[torch.tensor(neighbors[i]).to(self.device)])
            else:
                initial_hiddens.append(user_initial_embedding[torch.tensor(neighbors[i]).to(self.device)])
            offset+=1
    ...
    def stageOne(self, user: torch.Tensor, pos: torch.Tensor, neg: torch.Tensor):
        ...
        total_loss = 0
        for (user_neighbors, user_offsets, pos_neighbors, pos_offsets, neg_neighbors, neg_offsets) in result:
