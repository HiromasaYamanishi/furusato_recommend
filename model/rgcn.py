...
class LGCNConv(torch.nn.Module):
    def __init__(self, config, dataset):
        ...
        self.train_edge = torch.cat(
            [
                torch.stack([self.train_user, self.train_item]),
                torch.stack([self.train_item, self.train_user]),
            ],
            dim=1,
        ).to(config["device"])
        ...
        self.train_edge_div = torch.sqrt(
            self.all_div[self.train_edge[0]] * self.all_div[self.train_edge[1]]
        ).to(config["device"])
...
class RGCN(torch.nn.Module):
    def __init__(self, config, dataset):
        ...
        self.favorite_edge_index = torch.cat(
            [
                torch.stack(
                    [favoriteUser, favoriteItem + self.num_users],
                    dim=0,
                ),
                torch.stack(
                    [favoriteItem + self.num_users, favoriteUser],
                    dim=0,
                ),
            ],
            dim=1,
        ...
        self.relation_emb = torch.nn.Embedding(num_embeddings=2, embedding_dim=self.latent_dim)
    ...
    def __init_weight(self):
        self.all_embedding = torch.nn.Embedding(
            num_embeddings=self.num_items + self.num_users,
    ...
    def forward(self):
        ...
        x_out = x
        for i in range(self.num_layers):
            x = self.layers[i](x, self.edge_index)
            x_out = x_out + x
