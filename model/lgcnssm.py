...
class LightGCNSSM(torch.nn.Module):
    def __init__(self, config, dataset):
        super().__init__()
        self.dataset = dataset
        self.config = config
    ...
    def forward(self):
        ...
        x_out = x
        for i in range(self.num_layers):
            x = self.layers[i](x, self.train_edge)
            x_out = x_out + x
    ...
    def stageOne(self, user, pos, neg):
        ...
        loss, reg_loss = self.softmax_loss(user, pos, neg)
    ...
    def OneEpoch(self, user, pos, neg):
        ...
        for batch_i, (u, posItem, negItem) in tqdm(
            enumerate(
                minibatch(
                    user, pos, neg, batch_size=neg_size * self.config["bpr_batch_size"]
                )
            )
        ):
            u, posItem, negItem = (
