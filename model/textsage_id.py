...
#User/ItemのidつきGraphSAGE
class TextSAGE_ID(torch.nn.Module):
    def __init__(self, config, dataset):
        ...
        self.w_linears.append(
            nn.Linear(self.latent_dim * 4, self.latent_dim * 1)
        )  # TODO: change
    ...
    def init_parameters(self):
        ...
        nn.init.normal_(
            self.user_id_embeddings.weight,
        )
        nn.init.normal_(
            self.item_id_embeddings.weight,
        )
    ...
    def get_initial_emb(self, index):
        ...
        emb = torch.zeros((len(index), self.latent_dim * 2)).to(
            self.device
        )  # TODO: change
        # print(user, item)
