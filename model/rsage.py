...
#お気に入りやレビューなどのmulti-relationalな情報を考慮したモデル
class RSAGE(torch.nn.Module):
    def __init__(self, config, dataset):
        ...
        self.purchaseSize = self.edge_index.shape[1]
        self.config = config
        ...
        if self.config['multi_relational']=='sum':
            self.w_linears = nn.ModuleList(
                [
                    nn.Linear(self.latent_dim * 3, self.latent_dim * 1)
                    for _ in range(self.num_layers)
                ]
            )
        elif self.config['multi_relational']=='prod':
            self.w_linears = nn.ModuleList(
                [
        ...
        self.favorite_train = pd.read_csv(f'/home/yamanishi/project/furusato_recommend/data/favorite_train{suffix}.csv')
        ...
        self.favoriteSize = self.favorite_edge_index.shape[1]
        
        self.review_train = pd.read_csv(f'/home/yamanishi/project/furusato_recommend/data/review_train{suffix}.csv')
        reviewUser = torch.from_numpy(self.review_train['cf_customer'].values).long()
        reviewItem = torch.from_numpy(self.review_train['cf_product'].values).long()
        self.review_edge_index = torch.cat(
            [
                torch.stack(
                    [reviewUser, reviewItem + self.n_user],
                    dim=0,
                ),
                torch.stack(
                    [reviewItem + self.n_user, reviewUser],
                    dim=0,
                ),
            ],
            dim=1,
        )
        self.reviewSize = self.review_edge_index.shape[1]
        
        self.edge_index = torch.cat([self.edge_index, self.favorite_edge_index, self.review_edge_index],dim=1)
        self.edge_label = torch.zeros(self.edge_index.shape[1]).long()
        self.edge_label[self.purchaseSize:self.purchaseSize+self.favoriteSize] = 1
        self.edge_label[self.purchaseSize+self.favoriteSize:] = 2
        
        self.relation_embedding = torch.nn.Embedding(num_embeddings=3, embedding_dim=self.latent_dim)
    ...
    def propagate(self, x, relation_emb, edge_index, e_id, layer):
        edge_label = self.edge_label.to(self.device)[e_id]
        edge_relation_embedding = relation_emb[edge_label]
        source_x = x[edge_index[0]]
        if self.config['multi_relational']=='sum':
            source_x = torch.cat([source_x, edge_relation_embedding], dim=1)
        elif self.config['multi_relational']=='prod':
            source_x = source_x * edge_relation_embedding
        ...
        relation_emb = self.relation_linears[layer](relation_emb)
        if layer != self.num_layers - 1:
            x = x.relu()
        return x, relation_emb
    ...
    def forward(self, x, adjs):
        ...
        for i, (edge_index, e_id, size) in enumerate(adjs):
            x, relation_emb = self.propagate(x,relation_emb, edge_index, e_id, i)
            x = x[:size[1]]
        return x
    ...
    def getUsersRating(self, users):
        ...
        for i in range(self.num_layers):
            x ,relation_emb= self.propagate(x, relation_emb, self.edge_index.to(self.device), e_id.to(self.device), i)
