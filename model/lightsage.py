...
class LightSAGE(torch.nn.Module):
    ...
    def forward(self, x, adjs):
        ...
        for i, (edge_index, _, size) in enumerate(adjs):
            ...
            #    x = x.relu()
            x_out += x[:batch_size]
        return x_out/(self.num_layers+1)
    ...
    def getUsersRating(self, users):
        ...
        user_out, item_out = user_x, item_x
        for i in range(self.num_layers):
            ...
            user_x = user_x + user_x_agg
            item_x = item_x + item_x_agg
            user_out = user_out + user_x
            item_out = item_out + item_x
            # if i!=self.num_layers-1:
            #    item_x = item_x.relu()
            #    user_x = user_x.relu()
        # print(user_x.shape, item_x.shape)
        user_out = user_out/(self.num_layers+1)
        item_out = item_out/(self.num_layers+1)
        rating = torch.matmul(user_out[users], item_out.T)
        return rating
