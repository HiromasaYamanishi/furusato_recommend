...
class PinSAGE(torch.nn.Module):
    def __init__(self, config, dataset):
        ...
        for i in range(self.num_layers - 1):
            self.q_linears.append(nn.Linear(self.latent_dim * 1, self.latent_dim * 1))
        ...
        self.G1 = nn.Linear(self.latent_dim, self.latent_dim)
        self.G2 = nn.Linear(self.latent_dim, self.latent_dim, bias=False)
    ...
    def init_parameters(self):
        ...
        for i, w in enumerate(self.q_linears):
            if i == self.num_layers - 1:
                nn.init.xavier_uniform_(w.weight)
            else:
                nn.init.xavier_uniform_(w.weight, gain=gain)
    ...
    def propagate(self, x, edge_index, layer):
        source_x = self.q_linears[layer](x[edge_index[0]])
        source_x = self.dropout(source_x)
        target_x = x
        out = torch.zeros((x.shape[0], source_x.size(1))).to(self.device)
