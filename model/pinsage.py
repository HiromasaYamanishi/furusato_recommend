...
class PinSAGE(torch.nn.Module):
    def __init__(self, config, dataset):
        ...
        self.user_features = np.load(f'/home/yamanishi/project/furusato_recommend/data/cb/customer_feature_pad{suffix}.npy', allow_pickle=True)
        self.item_features = np.load(f'/home/yamanishi/project/furusato_recommend/data/cb/product_feature_pad{suffix}.npy', allow_pickle=True)
        self.item_text_features  = torch.from_numpy(np.load(f'/home/yamanishi/project/furusato_recommend/data/text/product_text_emb{suffix}.npy')).float().to(config['device'])
    ...
    def get_initial_embedding(self):
        user_initial_embedding = self.user_proj(F.embedding_bag(torch.tensor(self.user_feature_indices).to(self.device),
                                                 ...
                                                 mode='mean'))
        item_features = F.embedding_bag(torch.tensor(self.item_feature_indices).to(self.device),
        ...
        user_initial_embedding = torch.cat([self.user_id_embeddings.weight, user_initial_embedding], dim=1)
        item_initial_embedding = self.item_proj(torch.cat([item_features, item_text_features], dim=1))
        item_initial_embedding = torch.cat([self.item_id_embeddings.weight, item_initial_embedding], dim=1)
    ...
    def get_initial_train_emb(self, user, pos, neg):
        ...
        user_user_index, user_item_index = (user<self.n_user).to(self.device), (user>=self.n_user).to(self.device)
        pos_user_index, pos_item_index = (pos<self.n_user).to(self.device), (pos>=self.n_user).to(self.device)
        neg_user_index, neg_item_index = (neg<self.n_user).to(self.device), (neg>=self.n_user).to(self.device)
        user_user, user_item = user[user_user_index], user[user_item_index]-self.n_user
        ...
        user_x, pos_x, neg_x = torch.zeros((user.size(0), self.latent_dim*2)).to(self.device), torch.zeros((pos.size(0), self.latent_dim*2)).to(self.device), torch.zeros((neg.size(0), self.latent_dim*2)).to(self.device)
        user_x[user_user_index] = torch.cat([self.user_id_embeddings(user_user.to(self.device)), self.user_proj(torch.mean(self.user_feature_embeddings(torch.from_numpy(self.user_features[user_user]).to(self.device)), dim=1))], dim=1)
        user_x[user_item_index] = torch.cat([self.item_id_embeddings(user_item.to(self.device)),self.item_proj(torch.cat([torch.mean(self.item_feature_embeddings(torch.from_numpy(self.item_features[user_item]).to(self.device)), dim=1), self.item_text_features[user_item]], dim=1))], dim=1)
        pos_x[pos_user_index] = torch.cat([self.user_id_embeddings(pos_user.to(self.device)),self.user_proj(torch.mean(self.user_feature_embeddings(torch.from_numpy(self.user_features[pos_user]).to(self.device)), dim=1))], dim=1)
        pos_x[pos_item_index] = torch.cat([self.item_id_embeddings(pos_item.to(self.device)), self.item_proj(torch.cat([torch.mean(self.item_feature_embeddings(torch.from_numpy(self.item_features[pos_item]).to(self.device)), dim=1), self.item_text_features[pos_item]], dim=1))], dim=1)
