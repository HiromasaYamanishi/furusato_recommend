...
def save_result(config, dataset, product_names,  users_list, rating_list, groundTrue_list):
    ...
    wandb = config['wandb']
    suffix = config['suffix']
    df_save_name = f'./data/result/{model}/{model}_{recdim}_{layer}_multi_process.csv'
...
def demo(rank, world_size, gpu_index):
    ...
    for i in range(200):
        ...
        if i%TEST_SPAN==0 and rank==0:
            ...
            suffix = config['suffix']
            torch.save(ddp_model.module.state_dict(), f'/home/yamanishi/project/furusato_recommend/checkpoints/ddp_sage_{suffix}.pth')
