...
class Datas:
    def __init__(self, config):
        ...
        product = pd.read_pickle(f'./data/cb/{suffix}/product_cb{suffix}.pkl')
        customer = pd.read_pickle(f'./data/cb/{suffix}/customer_cb{suffix}.pkl')
        train = pd.read_pickle(f'./data/{suffix}/train{suffix}.pkl')
        test = pd.read_pickle(f'./data/{suffix}/test{suffix}.pkl')
        if suffix=='all':
            inference = pd.read_pickle(f'./data/{suffix}/inference{suffix}.pkl')
        print('loaded data')
        ...
        with open(f'./data/cf/{suffix}/allPos{suffix}.pkl', 'rb') as f:
...
def save_result(config, dataset, product_names,  users_list, rating_list, groundTrue_list):
    ...
    wandb = config['wandb']
    suffix = config['suffix']
    df_save_name = f'./data/result/{model}/{model}_{recdim}_{layer}_cstr_multi_process.csv'
...
class TextSAGE(torch.nn.Module):
    def __init__(self, config, dataset):
        ...
        if 'n' in self.config['user_feature']:
            self.user_numeric_features = torch.from_numpy(np.load(f'/home/yamanishi/project/furusato_recommend/data/cb/{suffix}customer_numeric_numpy{suffix}.npy'))
        if 'n' in self.config['item_feature']:
            self.item_numeric_features = torch.from_numpy(np.load(f'/home/yamanishi/project/furusato_recommend/data/cb/{suffix}/product_numeric_numpy{suffix}.npy'))
        ...
        with open(f"./data/text/{suffix}/user_name_tfidf{suffix}.pkl", "rb") as f:
            self.user_name = pickle.load(f)
        with open(f"./data/text/{suffix}/user_main_comment_tfidf{suffix}.pkl", "rb") as f:
            self.user_main_comment = pickle.load(f)
        with open(f"./data/text/{suffix}/user_main_list_comment_tfidf{suffix}.pkl", "rb") as f:
    ...
    def getUsersRating(self,):
        ...
        del user_initial_emb, item_initial_emb, trainItem, trainUser
...
def demo(rank, world_size, gpu_index):
    ...
    config['user_feature'] = 'ct'
    config['item_feature'] = 'ctsr'
    config['lr'] = 1e-3
    ...
    suffix = config['suffix']
    #
    model.load_state_dict(torch.load(f'/home/yamanishi/project/furusato_recommend/checkpoints/ddp_sage_ctsr{suffix}.pth'))
    ...
    for i in range(200):
        ...
        if i%TEST_SPAN==0 and rank==0:
            ...
            suffix = config['suffix']
            torch.save(ddp_model.module.state_dict(), f'/home/yamanishi/project/furusato_recommend/checkpoints/ddp_sage_ctsr{suffix}.pth')
